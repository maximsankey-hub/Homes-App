...
- [x] **Room-specific aesthetic photos → build a real style profile from swipes** — done (2026-07-19), but not live yet — see the "needs a Pexels key" note below.
  - New `StylePhoto` (global content catalog: room type + style name + image URL — not household-scoped, since a "Modern kitchen" photo is the same for everyone) and `StyleSwipe` (per-scorer liked/passed record) tables.
  - A one-time seed script (`npm run seed:style-photos -w server`) searches Pexels for each (room, style) pair, downloads 3 photos, and re-uploads them into Supabase Storage under a `style-photos/` prefix — gated behind `PEXELS_API_KEY`, safely no-ops with instructions if it's not set. Taxonomy seeded: Kitchen/Bathroom/Primary bed/Backyard, 3 styles each (e.g. Kitchen: Modern, Farmhouse, Traditional).
  - **Needs a Pexels key to actually have photos** — `PEXELS_API_KEY` isn't set anywhere yet (free, instant signup at pexels.com/api). Until it's added and the seed script is run once, the "Style profile" section on the Profile screen shows an empty state and there's nothing to swipe.
  - New "Style profile" section on the Profile screen lists each room with photos, showing swipe progress and a detected leaning once there's a clear signal (opens a swipe modal reusing the existing kitchen swipe-card visual, now with real photos instead of emoji). A room's "preferred style" only surfaces once one style has at least 2 likes and is strictly ahead of every other style in that room — otherwise it's too little signal to be worth showing.
  - Finishing a room's deck with a clear leader offers "Add as a priority for {room}" — accepting creates a real `CustomMetric` (e.g. "Modern aesthetic (Kitchen)", scoped to that room via `targetRoomName`), directly reusing the priorities/custom-metric system.
...
- [x] **Auto-fill property details from an address** — done (2026-07-19) via RentCast's free tier, but not live yet. Added an "Auto-fill from address" button on the add/edit property form that calls a new `GET /properties/lookup?address=` endpoint (`server/src/services/rentcast.ts`), filling in sq ft, beds, baths, year built, and showing an estimated value as a reference (list price still stays manual — that's private MLS data RentCast doesn't have). Also added a year-built field to the form, which didn't exist before. **Needs `RENTCAST_API_KEY`** (free tier, ~50 requests/month, sign up at app.rentcast.io/app/api) — not set anywhere yet; the endpoint safely returns nothing until it is.
...
- [x] **Let users add a custom metric within any topic, not just the generic "Custom scoring metrics" card** — added a "+ Add your own question" row at the bottom of every `PriorityGroupCard`. Metrics added this way get a new `CustomMetric.topicKey` tagging which topic they came from (e.g. `walkability`), so they render alongside the curated catalog items in that same topic instead of only in the separate bottom-of-page section — while catalog items and the fully-generic Profile-screen additions both keep `topicKey: null`.
