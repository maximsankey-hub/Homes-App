...
- [ ] **Add a renovation idea from live Visit mode** — the renovation-idea form (cost estimate — AI or manual, photo attachment) already exists on the property detail's Visit *sub-tab* (post-visit review screen), but not in the live Visit *mode* flow (bottom nav "Visit" → room picker → scoring steps). Add the same capability while actively scoring a room during a visit, pre-tagged to whichever room you're currently on, so you don't have to leave the flow and come back later to jot down a renovation thought.
- [ ] **Forgot password / reset password flow** — came up when a test account's password needed changing; there's currently no in-app way to reset a forgotten password, only editing directly in the Supabase dashboard (which can also reset email-confirmation status and lock the account out further). Add a standard "forgot password" link on the login screen using Supabase Auth's password-reset email flow.
