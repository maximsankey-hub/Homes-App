...
- [x] **User-defined custom scoring metrics** — let the user add their own scoring metric (e.g. "accessibility"), alongside the built-in ones like natural light/storage, that can be scored per-room. When creating a custom metric, the user classifies it as either **emotional** or **functional**, and it factors into the room's emotional/functional averages alongside the built-ins. Setup/creation of new custom metrics only happens from the profile screen, which also got a weighting slider per custom metric (matching the existing built-in weight sliders). Property-level (not just room-level) custom scoring was considered but scoped out — could be a follow-up if wanted.
...
- [x] **Include neighborhood appeal in score weighting** — added a `weightNeighborhood` slider next to the existing weighting sliders (Profile screen and onboarding wizard). Note: like the pre-existing `weightEmotional`/`weightStorage`/`weightLight` sliders, this is stored as a declared preference but isn't actually wired into the property-score math yet — none of the weight sliders are. Actually feeding weights into the score formula would be a separate, larger change across all four.
